...
Accelerate library can be formally tested for XPU backend on the system with the supported Intel GPU graphics with the project included tests:
...
| Accelerate | PyTorch | Failed | Passed | Skipped |
| --- | --- | --- | --- | --- |
| `v0.34.2` | `v2.5.0` | 19 | 214 | 142 |

PyTorch 2.5 includes a fix for [pytorch/pytorch#127929] which unlocked Accelerate tests. On `v0.34.2` passing rate is 91.8% with 19 tests failed, 214 passed and 142 skipped (executed on a system with a single [Intel® Data Center GPU Max 1100] card). The most significant issue affecting majority of tests (16 out of 19) is [pytorch/pytorch#135550] on PyTorch side. 2 changes are needed to address this issue, one on PyTorch side – [pytorch/pytorch#135567], another in Triton – [intel/intel-xpu-backend-for-triton#2192] (see [pytorch/pytorch#137886] for Triton commit pin update in PyTorch):

Skipped tests require multiple GPUs, XPU enabled `bitsandbytes` library (missing) and/or other missing libraries. Thes need further attention and enabling efforts.
